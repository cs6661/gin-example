...
var httpCnt = &Metric{
	ID:          "httpCnt",
	Name:        "http_api_total",
	Description: "all HTTP requests processed count, partitioned by status code and HTTP method.",
	Type:        "counter_vec",
	Args:        []string{"code"},
}
...
// prometheus_collector.Collector type (i.e. CounterVec, Summary, etc) of each metric
type Metric struct {
...
type Prometheus struct {
	...
	// gin.Context string to use as a prometheus_collector URL label
...
type PrometheusPushGateway struct {
	...
	// if implemented using prometheus_collector common/expfmt instead
	MetricsURL string
...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if _, err = client.Do(req); err != nil {
		fmt.Println("Error sending to push gateway" + err.Error())
...
// NewMetric associates prometheus_collector.Collector based on Metric.Type
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			fmt.Println("Error registering metric" + err.Error())
			//log.WithError(err).Errorf("%s could not be registered in Prometheus", metricDef.Name)
		}
		switch metricDef {
		case httpCnt:
			p.httpCnt = metric.(*prometheus.CounterVec)
		case reqCnt:
...
func (p *Prometheus) HandlerFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//	url = u.(string)
		//}
		p.httpCnt.WithLabelValues(status).Inc()
